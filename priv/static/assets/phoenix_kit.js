...
(function() {
  ...
  // =====================================================
  // SORTABLE MODULE
  // Auto-loads SortableJS from CDN for drag-and-drop grids
  // =====================================================
  ...
  (function() {
    ...
    function loadConsent() {
      ...
      } catch (e) { log("Could not load consent", e); }
      return null;
    }
    ...
    function saveConsent(consent) {
      ...
      } catch (e) { log("Could not save consent", e); }
    }
    ...
    function setupCrossTabSync() {
      window.addEventListener("storage", function(e) {
        if (e.key === STORAGE_KEY && e.newValue) {
          ...
          } catch (err) { log("Cross-tab sync error", err); }
        }
      });
    }
    ...
    function unblockScripts(category) {
      var scripts = document.querySelectorAll('script[data-consent-category="' + category + '"][data-blocked="true"]');
      scripts.forEach(function(script) {
      ...
      if (scripts.length > 0) log("Scripts unblocked for category: " + category);
    }
    ...
    function applyConsent(consent) {
      ...
      window.dispatchEvent(new CustomEvent("phx:consent-updated", { detail: { consent: consent } }));
    }
    ...
    function createWidgetHTML(config) {
      ...
      return '<div id="pk-consent-root" class="pk-consent-widget">' +
        '<style>' +
          '.pk-consent-widget{--pk-bg:oklch(var(--b1));--pk-bg-alt:oklch(var(--b2));--pk-border:oklch(var(--b3));--pk-text:oklch(var(--bc));--pk-text-muted:oklch(var(--bc)/0.6);--pk-primary:oklch(var(--p));--pk-primary-content:oklch(var(--pc));--pk-primary-soft:oklch(var(--p)/0.1);--pk-primary-glow:oklch(var(--p)/0.4);--pk-shadow:0 8px 32px oklch(var(--bc)/0.12);}' +
        ...
        '</style>' +
        (showIcon ?
          '<button id="pk-consent-icon" type="button" onclick="window.PhoenixKitConsent.openPreferences()" class="pk-floating-icon pk-glass" style="position:fixed;z-index:50;width:3rem;height:3rem;border-radius:9999px;display:flex;align-items:center;justify-content:center;cursor:pointer;background:var(--pk-primary);' + iconStyle + '" aria-label="Cookie preferences" title="Cookie preferences">' +
            '<svg style="width:1.5rem;height:1.5rem;color:var(--pk-primary-content)" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>' +
          '</button>' : '') +
          ...
          '<div style="max-width:64rem;margin:0 auto;padding:1rem 1.5rem">' +
            '<div style="display:flex;flex-wrap:wrap;align-items:center;gap:1rem">' +
              '<div style="flex:1;display:flex;align-items:flex-start;gap:0.75rem;min-width:200px">' +
                '<div style="flex-shrink:0;width:2.5rem;height:2.5rem;border-radius:9999px;background:var(--pk-primary-soft);display:flex;align-items:center;justify-content:center"><svg style="width:1.25rem;height:1.25rem;color:var(--pk-primary)" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93z"/></svg></div>' +
                '<div><h3 style="font-weight:600;font-size:0.875rem;margin:0;color:var(--pk-text)">We value your privacy</h3><p style="font-size:0.75rem;color:var(--pk-text-muted);margin:0.25rem 0 0 0">We use cookies to enhance your experience. <a href="' + (config.cookie_policy_url || '/legal/cookie-policy') + '" style="color:var(--pk-primary);text-decoration:underline" target="_blank">Cookie Policy</a></p></div>' +
              '</div>' +
          ...
          '<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:1rem;pointer-events:none">' +
            '<div class="pk-modal-content pk-glass" style="width:100%;max-width:28rem;max-height:85vh;overflow:hidden;border-radius:1rem;pointer-events:auto">' +
              '<div style="display:flex;align-items:center;justify-content:space-between;padding:1rem 1.5rem;border-bottom:1px solid var(--pk-border)">' +
                '<div style="display:flex;align-items:center;gap:0.75rem;flex:1"><div style="width:2.5rem;height:2.5rem;border-radius:9999px;background:var(--pk-primary-soft);display:flex;align-items:center;justify-content:center"><svg style="width:1.25rem;height:1.25rem;color:var(--pk-primary)" viewBox="0 0 24 24" fill="currentColor"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 10.99h7c-.53 4.12-3.28 7.79-7 8.94V12H5V6.3l7-3.11v8.8z"/></svg></div><div><h2 style="font-weight:600;font-size:1.125rem;margin:0;color:var(--pk-text)">Privacy Preferences</h2><p style="font-size:0.75rem;color:var(--pk-text-muted);margin:0">Manage your cookie settings</p></div></div>' +
                '<button type="button" onclick="window.PhoenixKitConsent.closePreferences()" class="btn btn-ghost btn-sm btn-circle" aria-label="Close"><svg style="width:1.25rem;height:1.25rem" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>' +
              '</div>' +
              ...
              '<div style="padding:1rem 1.5rem;border-top:1px solid var(--pk-border);background:var(--pk-bg-alt)">' +
                '<div style="display:flex;flex-wrap:wrap;align-items:center;gap:0.75rem">' +
                  '<div style="font-size:0.75rem;color:var(--pk-text-muted)"><a href="' + (config.privacy_policy_url || '/legal/privacy-policy') + '" style="color:inherit;text-decoration:underline" target="_blank">Privacy Policy</a> • <a href="' + (config.cookie_policy_url || '/legal/cookie-policy') + '" style="color:inherit;text-decoration:underline" target="_blank">Cookie Policy</a></div>' +
                  '<div style="margin-left:auto;display:flex;gap:0.5rem"><button type="button" onclick="window.PhoenixKitConsent.rejectAll()" class="btn btn-ghost btn-sm" style="font-size:0.75rem">Reject All</button><button type="button" onclick="window.PhoenixKitConsent.savePreferences()" class="btn btn-primary btn-sm" style="font-size:0.75rem">Save Preferences</button></div>' +
                '</div>' +
              '</div>' +
            '</div>' +
          '</div>' +
    ...
    function showIcon() {
      ...
      if (icon && isOptInMode()) { icon.style.display = "flex"; icon.style.opacity = "1"; }
    }
    ...
    function showModal() {
      ...
      if (modal) {
        ...
        hideBanner();
        setTimeout(function() { var firstButton = modal.querySelector("button"); if (firstButton) firstButton.focus(); }, 100);
      }
    }
    ...
    function updateCheckboxes() {
      ...
      CATEGORIES.forEach(function(category) {
        ...
        if (checkbox && !checkbox.disabled) checkbox.checked = !!consent[category];
      });
    }
    ...
    function updateUI() {
      if (shouldShowBanner()) { showBanner(); hideIcon(); }
      else { hideBanner(); showIcon(); }
      updateCheckboxes();
    }

    PhoenixKitConsent.acceptAll = function() {
      var consent = { necessary: true, analytics: true, marketing: true, preferences: true, timestamp: new Date().toISOString() };
      PhoenixKitConsent.consent = consent;
      saveConsent(consent);
      applyConsent(consent);
      hideBanner(); hideModal(); showIcon();
      log("All cookies accepted");
    };

    PhoenixKitConsent.rejectAll = function() {
      var consent = { necessary: true, analytics: false, marketing: false, preferences: false, timestamp: new Date().toISOString() };
      PhoenixKitConsent.consent = consent;
      saveConsent(consent);
      applyConsent(consent);
      hideBanner(); hideModal(); showIcon();
    ...
    PhoenixKitConsent.openPreferences = function() { updateCheckboxes(); showModal(); log("Preferences modal opened"); };
    PhoenixKitConsent.closePreferences = function() { hideModal(); log("Preferences modal closed"); };
    PhoenixKitConsent.getConsent = function() { return PhoenixKitConsent.consent; };
    PhoenixKitConsent.hasConsent = function(category) { return PhoenixKitConsent.consent && !!PhoenixKitConsent.consent[category]; };
    PhoenixKitConsent.revokeConsent = function() {
      try {
        ...
        updateUI();
        log("Consent revoked");
      } catch (e) { log("Could not revoke consent", e); }
    };

    function initFromConfig(config) {
      if (config.should_show === false) { log("Widget hidden (user authenticated or disabled)"); return; }
      PhoenixKitConsent.config = {
        frameworks: config.frameworks || [], consentMode: config.consent_mode || "strict",
        policyVersion: config.policy_version || "1.0", googleConsentMode: config.google_consent_mode || false,
        iconPosition: config.icon_position || "bottom-right", showIcon: config.show_icon || false,
      ...
      PhoenixKitConsent.elements = {
        root: document.getElementById("pk-consent-root"), icon: document.getElementById("pk-consent-icon"),
        banner: document.getElementById("pk-consent-banner"), modal: document.getElementById("pk-consent-modal")
      };
      if (PhoenixKitConsent.config.consentMode === "strict") initGoogleConsentMode();
      setupCrossTabSync();
      var stored = loadConsent();
      if (stored) { PhoenixKitConsent.consent = stored; applyConsent(stored); }
      else if (isOptInMode() && PhoenixKitConsent.config.consentMode === "strict") blockScripts();
      updateUI();
    ...
    function initFromElement(rootElement) {
      ...
      PhoenixKitConsent.config = {
        frameworks: config.frameworks, consentMode: config.consent_mode, policyVersion: config.policy_version,
        googleConsentMode: config.google_consent_mode, iconPosition: config.icon_position, showIcon: config.show_icon,
        cookiePolicyUrl: config.cookie_policy_url, privacyPolicyUrl: config.privacy_policy_url
      };
      PhoenixKitConsent.elements = {
        root: rootElement, icon: document.getElementById("pk-consent-icon"),
        banner: document.getElementById("pk-consent-banner"), modal: document.getElementById("pk-consent-modal")
      };
      if (PhoenixKitConsent.config.consentMode === "strict") initGoogleConsentMode();
      setupCrossTabSync();
    ...
    function fetchConfigAndInit() {
      fetch(getConfigEndpoint(), { credentials: "same-origin" })
        .then(function(response) { if (!response.ok) throw new Error("Config endpoint returned " + response.status); return response.json(); })
        .then(function(config) {
          if (config.enabled && config.should_show !== false) initFromConfig(config);
          else { log("Consent widget is disabled"); resetGoogleConsentMode(); }
        })
        .catch(function(err) { log("Could not fetch config (widget disabled or endpoint unavailable)", err); });
    }

    // LiveView Hook
    window.PhoenixKitHooks.CookieConsent = {
      mounted: function() { if (!PhoenixKitConsent.initialized) initFromElement(this.el); },
      destroyed: function() { PhoenixKitConsent.initialized = false; }
    };
    ...
    document.addEventListener("DOMContentLoaded", function() {
      ...
      if (existingRoot) { log("Widget already in DOM, waiting for LiveView hook"); return; }
      fetchConfigAndInit();
    });
  })();
  ...
  // =====================================================
  // RESET SELECT HOOK
  // Resets select element to first option on push event
  // =====================================================

  window.PhoenixKitHooks.ResetSelect = {
    mounted() {
      this.handleEvent("reset_select", ({id}) => {
        if (this.el.id === id) this.el.selectedIndex = 0;
      });
    }
  };
  ...
  // =====================================================
  // TIME AGO HOOK
  // Client-side relative time updates with variable interval
  // =====================================================
  ...
  window.PhoenixKitHooks.TimeAgo = {
    mounted() {
      ...
      if (isNaN(parsed.getTime())) { console.warn("TimeAgo: Invalid timestamp", timestamp); return; }
    ...
    disconnected() { this.clearTimer(); },
    reconnected() { if (this.timestamp) { this.update(); this.scheduleUpdate(); } },
    updated() {
    ...
    scheduleUpdate() {
      ...
      this.timer = setTimeout(() => { this.update(); this.scheduleUpdate(); }, interval);
    },
    update() {
      const text = this.getRelativeTime();
      if (text && this.el.textContent !== text) this.el.textContent = text;
    },
    getRelativeTime() {
      const now = Date.now();
