...
```elixir
# Create order
{:ok, order} = PhoenixKit.Billing.create_order(user, %{
  currency: "EUR",
  payment_method: "bank",
  line_items: [
...
order = PhoenixKit.Billing.get_order(id, preload: [:user, :billing_profile])

# List orders with pagination
{orders, total} = PhoenixKit.Billing.list_orders_with_count(
  page: 1,
  per_page: 25,
  status: "confirmed",
...
# Update order status
{:ok, order} = PhoenixKit.Billing.confirm_order(order)
{:ok, order} = PhoenixKit.Billing.mark_order_paid(order)
{:ok, order} = PhoenixKit.Billing.cancel_order(order)
```
...
{:ok, invoice} = PhoenixKit.Billing.create_invoice_from_order(order)

# Update invoice status
{:ok, invoice} = PhoenixKit.Billing.send_invoice(invoice)
{:ok, invoice} = PhoenixKit.Billing.mark_invoice_paid(invoice)
{:ok, invoice} = PhoenixKit.Billing.void_invoice(invoice)

# Generate receipt after payment
{:ok, invoice} = PhoenixKit.Billing.generate_receipt(invoice)

# List invoices with pagination
{invoices, total} = PhoenixKit.Billing.list_invoices_with_count(
  page: 1,
  per_page: 25,
  status: "sent"
)
...
{:ok, profile} = PhoenixKit.Billing.create_billing_profile(user, %{
  type: "individual",
  first_name: "John",
  last_name: "Doe",
...
profiles = PhoenixKit.Billing.list_user_billing_profiles(user_id)

# Set default profile
{:ok, profile} = PhoenixKit.Billing.set_default_billing_profile(profile)
```
...
currencies = PhoenixKit.Billing.list_currencies(enabled: true)

# Get default currency
currency = PhoenixKit.Billing.get_default_currency()

# Update currency
{:ok, currency} = PhoenixKit.Billing.update_currency(currency, %{
...
PhoenixKit.Billing.Events.subscribe()
