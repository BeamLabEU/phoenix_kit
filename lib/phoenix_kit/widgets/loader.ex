defmodule PhoenixKit.Widgets.Loader do
  @moduledoc """
  ...
  # List of all known Phoenix Kit modules
  @known_modules [
    ...
    PhoenixKit.Modules.Media,
    PhoenixKit.Modules.Email
  ]
      ...
      widgets_module ->
        try do
          if function_exported?(widgets_module, :widgets, 0) do
            ...
            |> List.wrap()
            |> Enum.flat_map(fn w -> List.wrap(w) end)
            |> Enum.map(&ensure_widget_struct/1)
            |> Enum.map(&annotate_widget(&1, module_name))
            |> Enum.filter(& &1.enabled)
          else
            Logger.warn("Widget module #{inspect(widgets_module)} does not export widgets/0")
            []
          end
        rescue
          e ->
  ...
  defp user_can_access_module?(user, module_name) do
    ...
    # Example: PhoenixKit.Users.Permissions.user_can_access_module?(user, module_name)
    true
  end
