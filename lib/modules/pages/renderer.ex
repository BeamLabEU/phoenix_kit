defmodule PhoenixKit.Modules.Pages.Renderer do
  @moduledoc """
  ...
  @global_cache_key "pages_render_cache_enabled"
  @per_blog_cache_prefix "pages_render_cache_enabled_"
  ...
  Respects `pages_render_cache_enabled` (global) and
  `pages_render_cache_enabled_{blog_slug}` (per-blog) settings.
  ...
  def render_markdown(content) when is_binary(content) do
    ...
    Logger.debug("Content render time: #{time}us", content_size: byte_size(content))
    result
  end
