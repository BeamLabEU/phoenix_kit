defmodule PhoenixKit.Modules.Pages.Storage.Paths do
  @moduledoc """
  ...
  def root_path do
    ...
    new_path = Path.join(base_priv, "pages")
  ...
  @doc """
  Returns the path for a specific pages group, checking both new and legacy locations.
  ...
  def group_path(group_slug) do
    base_priv = get_parent_app_priv()
    new_group_path = Path.join([base_priv, "pages", group_slug])
  ...
  Always returns the new "pages" path.
  """
  @spec write_root_path() :: String.t()
  def write_root_path do
    base_priv = get_parent_app_priv()
    path = Path.join(base_priv, "pages")
    File.mkdir_p!(path)
    path
  end
  ...
  @doc """
  Migrates a pages group from the legacy "blogging" directory to the new "pages" directory.
  ...
  @doc """
  Ensures the folder for a pages group exists.
  For new groups, creates in the new "pages" directory.
  ...
  Stops at the pages/blogging root.
  """
    ...
    parent_app =
      case PhoenixKit.Config.get_parent_app() do
        nil ->
          ...
          Cannot determine storage path for pages module.
